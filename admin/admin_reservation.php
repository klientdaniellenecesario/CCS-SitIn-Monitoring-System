...
// Handle approve reservation - carries over purpose, sets time_in, decrements session count
if (isset($_POST['approve'])) {
    ...
    // Create sit-in session from approved reservation with purpose, time_in, and pc_number
    ...
    if (mysqli_query($conn, $insert)) {
        // DECREMENT session count since student is using a session
        ...
        $error = "Failed to create sit-in session: " . mysqli_error($conn);
    }
    header('Location: admin_reservation.php?lab=' . $lab);
    exit();
}
...
<head>
    ...
    <style>
        ...
        .pc-card.available { border-color: #cbd5e0; }
                                    ...
                                    <td class="actions">
                                        <?php if($res['status'] == 'pending'): ?>
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" name="approve" class="btn-approve" onclick="return confirm('Approve this reservation?')">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <input type="hidden" name="pc_number" value="<?php echo $res['pc_number']; ?>">
                                                <input type="hidden" name="date" value="<?php echo $res['date']; ?>">
                                                <button type="submit" name="reject" class="btn-reject" onclick="return confirm('Reject this reservation?')">
                                        ...
                                        <form method="POST" style="display: inline;">
                                            <input type="hidden" name="reservation_id" value="<?php echo $res['id']; ?>">
                                            <button type="submit" name="delete" class="btn-delete" onclick="return confirm('Delete this reservation?')">
            ...
            fetch('../backend/get_pc_status.php?lab=' + encodeURIComponent(lab))
                ...
                .then(data => {
                    ...
                    for (let i = 1; i <= 30; i++) {
                        ...
                        let statusClass = '', statusText = '', iconColor = '', userHtml = '', actionsHtml = '';
                        ...
                        if (status.status === 'available') {
                            ...
                            actionsHtml = `<div class="pc-actions">
                                <button class="btn-end-session" onclick="endSession(${status.session_id}, '${pcNumber}', '${lab}')">End Session</button>
                            </div>`;
                            ...
                            userHtml = `<div class="pc-user"><i class="fas fa-user"></i> ${status.user || 'Unknown'}</div>`;
                            actionsHtml = `<div class="pc-actions">
                                <button class="btn-approve-pc" onclick="approveReservation(${status.reservation_id}, ${status.user_id}, '${pcNumber}', '${lab}')">Approve</button>
                                <button class="btn-reject-pc" onclick="rejectReservation(${status.reservation_id}, ${status.user_id}, '${pcNumber}', '${lab}')">Reject</button>
                            </div>`;
                        }
